%% Homework 4: Question 1: Multi-Layer Perceptrons (MLP) Classifier
clear all; close all;
...
for M = 1:10
        ...
        fprintf('N=%d, Hidden=%d, Mean Val Error=%.3f, Mean Val Accuacy=%.3f \n', Ntrain, numPerceptrons, meanValError(M)*100, 100-(meanValError(M)*100));
        
        figure(4);
        subplot(2,1,1)
        scatter(numPerceptrons, meanValError(M)*100, 50, 'r', 'filled')
        ylim([20 40])
        hold on
        xlabel("Number of Perceptrons in Hidden Layer")
        ylabel("Error (%)")
        subplot(2,1,2)
        scatter(numPerceptrons, 100-meanValError(M)*100, 'b', 'filled')
        hold on
        xlabel("Number of Perceptrons in Hidden Layer")
        ylabel("Accuracy (%)")
        ylim([60 90])
